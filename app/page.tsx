...
export default function Home() {
  const [screen, setScreen] = useState<'upload' | 'annotate'>('upload');
  ...
  const [scores, setScores] = useState<Record<number, Record<string, number | string>>>({});
    ...
    const tryRender = () => {
      ...
      if (w.renderMathInElement) {
        [questionRef.current, solutionRef.current].forEach(el => {
          if (el) {
            (w.renderMathInElement as (el: HTMLElement, opts: unknown) => void)(el, {
              delimiters: [
  ...
  function renderMD(text: string) {
    ...
    const protect = (m: string) => { mathBlocks.push(m); return TOKEN + (mathBlocks.length - 1) + TOKEND; };
    let s = text;
  ...
  function handleFile(file: File) {
    ...
    reader.onload = e => {
      try {
        const w = window as unknown as Record<string, unknown>;
        const XLSX = w.XLSX as { read: (data: unknown, opts: unknown) => unknown; utils: Record<string, unknown> };
        if (!XLSX) { setUploadError('File parser still loading, please try again in a second.'); return; }
        const wb = (XLSX.read as (data: unknown, opts: unknown) => { Sheets: Record<string, unknown>; SheetNames: string[] })(e.target?.result, { type: 'array' });
        const ws = wb.Sheets[wb.SheetNames[0]];
        const raw2D = (XLSX.utils['sheet_to_json'] as (ws: unknown, opts: unknown) => string[][])(ws, { header: 1, defval: '' });
        ...
        const findCol = (...keys: string[]) => headers.find(h => keys.some(k => lc(h).includes(k))) || headers[0];
        ...
        if (saved) { try { const p = JSON.parse(saved); setScores(p.scores||{}); setNotes(p.notes||{}); } catch(e) { console.error(e); } }
      } catch(err) { setUploadError('Could not read this file: '+(err as Error).message); }
    };
  ...
  function persist(s: typeof scores, n: typeof notes) {
    try {
  ...
  function exportXLSX() {
    ...
    const w = window as unknown as Record<string, unknown>;
    const XLSX = w.XLSX as { utils: Record<string, unknown>; writeFile: (wb: unknown, name: string) => void };
    if (!XLSX) return;
    ...
    const ws = (XLSX.utils['json_to_sheet'] as (rows: unknown) => unknown)(outputRows);
    (XLSX.utils['book_append_sheet'] as (wb: unknown, ws: unknown, name: string) => void)(wb, ws, 'Annotations');
  ...
  const sbCls = (dim: string, val: number) => {
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .tb-brand{font-family:var(--mono);font-size:12px;letter-spacing:2px;text-transform:uppercase;color:var(--accent);white-space:nowrap}
        .tb-file{font-size:12px;color:var(--text-dim);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:200px}
        .tb-meta{font-size:11px;color:var(--accent2);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:280px;font-family:var(--mono)}
        ...
        .btn-export.incomplete:hover{background:#e09010}
      ...
      {screen === 'upload' && (
        <div className="upload-screen">
          ...
          <h1 className="upload-title">Math Solution Annotation</h1>
          <p className="upload-sub">Upload your Excel spreadsheet. Header rows and instructions are auto-detected and skipped. Score each solution on 7 quality dimensions.</p>
        ...
        <div>
          <div id="topbar">
            <div className="tb-left">
              <span className="tb-brand">Annotate</span>
              <span className="tb-file">{fileName}</span>
              {metaLines.length>0 && <span className="tb-meta">{metaLines[0].length>80?metaLines[0].slice(0,80)+'…':metaLines[0]}</span>}
          ...
          <div id="split-wrap">
            <div id="left-panel" ref={leftPanelRef}>
              <div className="card">
                ...
                <div className="card-body" ref={questionRef} dangerouslySetInnerHTML={{__html: renderMD(String(row[colQ]??''))}} />
              </div>
              <div className="card">
                <div className="card-head"><span className="card-tag tag-s">Solution</span></div>
                <div className="card-body" ref={solutionRef} dangerouslySetInnerHTML={{__html: renderMD(String(row[colS]??''))}} />
              </div>
              <div id="bottom-nav">
